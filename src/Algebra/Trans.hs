...
instance Algebra m => AlgebraTrans (E.ExceptT e) m where
  ...
  algT = \case
    ...
    R (Catch m h) -> lowerWith (\ lower -> E.catchE (lower m) (lower . h))
...
instance Algebra m => AlgebraTrans (R.ReaderT r) m where
  ...
  algT = \case
    Ask       -> liftInit R.ask
    Local f m -> mapLowerT (R.local f) id . lower m
...
instance Algebra m => AlgebraTrans (S.L.StateT s) m where
  ...
  algT = \case
    Get   -> liftInit S.L.get
    Put s -> liftInit (S.L.put s)
...
instance Algebra m => AlgebraTrans (S.S.StateT s) m where
  ...
  algT = \case
    Get   -> liftInit S.S.get
    Put s -> liftInit (S.S.put s)
