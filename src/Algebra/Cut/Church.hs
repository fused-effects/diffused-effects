...
instance Algebra m => Algebra (CutT m) where
  ...
  alg (ctx :: ctx ()) (hdl :: forall x . ctx (n x) -> CutT m (ctx x)) = \case
    ...
    R (R other)          -> CutT $ \ cons nil fail -> thread (pure ctx) dst other >>= runIdentity . runCut (coerce cons) (coerce nil) (coerce fail)
    where
